"""
Ablation Study 2: Non-Evolving Fixed Weights

This version uses fixed weights based on dataset size ratio throughout training.
Weights are computed once based on sqrt(N_large/N_small) and remain constant.

Key modifications from TF-DWT:
- Weights are fixed (no warmup, no evolution)
- w_small = sqrt(N_large / N_small), capped at [1.5, 3.0]
- Keep MMD alignment (with warmup)
- Keep Split-BN
...
def ablation2_train_fold(
    ...
    """Ablation 2: Non-evolving fixed weights based on dataset size"""
    device = get_device()
    ...
    _, lambda_mmd_target, warmup_epochs = get_symmetric_adjustments(n_train_avo, n_train_p3)
    ...
    # ABLATION 2: Compute fixed weights (no evolution, no warmup for weights)
    n_small = n_train_p3 if small_domain == 'P3' else n_train_avo
    n_large = n_train_avo if small_domain == 'P3' else n_train_p3
    w_small_fixed = max(1.5, min(3.0, math.sqrt(max(1, n_large) / max(1, n_small))))
    w_large_fixed = 1.0
    ...
    logger.info(f"ABLATION 2: Fixed weights (w_{small_domain}={w_small_fixed:.3f}, w_{large_domain}={w_large_fixed:.3f}), lambda_MMD_target={lambda_mmd_target:.3f}, warmup_epochs={warmup_epochs}")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # ABLATION 2: Fixed weights (no warmup for weights)
        w_small = w_small_fixed
        w_large = w_large_fixed

        # Warmup for MMD only (keep this as it's orthogonal to weight evolution)
...
def run_nested_cv_ablation2(logger: logging.Logger, channels: List[str]) -> Dict[str, float]:
    ...
    for repeat in range(NESTED_CV_REPEATS):
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X_all, y_all)):
            ...
            # Train fold model with ablation 2
            model = ablation2_train_fold(
                logger,
    ...
    log_section_header(logger, "CROSS-VALIDATION RESULTS (ABLATION 2: FIXED WEIGHTS)")
...
def main():
    ...
    try:
        ...
        # Logger setup
        logger = setup_logger('Ablation2_AVOsmall_FixedWeights', create_file=True)
        log_section_header(logger, "ABLATION 2: Non-Evolving Fixed Weights")

        # Log configuration summary
        log_configuration(logger, {
            'ablation': 'Fixed Weights (no evolution, based on dataset size)',
        ...
        # Run Ablation 2 with proper cross-validation
        log_section_header(logger, "Running Nested Cross-Validation with Ablation 2")
        results = run_nested_cv_ablation2(logger, channels)
        ...
        if 'detailed_fold_results' in results:
            ...
            csv_filename = f'ablation_results_AVOsmall/ablation2_fixed_weights_detailed_{timestamp}.csv'
            ...
            summary_filename = f'ablation_results_AVOsmall/ablation2_fixed_weights_summary_{timestamp}.csv'
            summary_df.to_csv(summary_filename, index=False)
            logger.info(f"Summary statistics saved to: {summary_filename}")
            print(f"Summary statistics saved to: {summary_filename}")

        print("\\n--- Ablation 2 Complete (Fixed Weights) ---")
        ...
        print(f"\\n--- Ablation 2 Experiment Failed: {e} ---")
        if logger:
            cleanup_failed_log(logger)
        raise
    except KeyboardInterrupt:
        print("\\n--- Ablation 2 Experiment Interrupted by User ---")
        if logger:
            cleanup_failed_log(logger)
        raise
