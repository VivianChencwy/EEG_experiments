"""
Ablation Study 1: Equal Weights for All Trials

This version removes the domain-specific weighting mechanism.
Both P3 and AVO losses are weighted equally (w_P3 = w_AVO = 1.0).

Key modifications from TF-DWT:
- Set w_small = w_large = 1.0 (no domain weighting)
- Keep MMD alignment
- Keep Split-BN
...
def ablation1_train_fold(
    ...
    """Ablation 1: Equal weights for all trials (w_P3 = w_AVO = 1.0)"""
    device = get_device()
    ...
    logger.info(f"ABLATION 1: Equal weights (w_P3=w_AVO=1.0), lambda_MMD_target={lambda_mmd_target:.3f}, warmup_epochs={warmup_epochs}")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # ABLATION 1: Equal weights (no domain weighting)
        w_p3 = 1.0
        w_avo = 1.0
        ...
        logger.info(f"Epoch {epoch}/{MAX_EPOCHS} | LR={lr_cur:.6f} | w_P3={w_p3:.3f} | w_AVO={w_avo:.3f} | lambda_MMD={lambda_mmd:.3f}")
        ...
        while True:
            ...
            # ABLATION 1: Use equal weights (w_p3 and w_avo determined by which is small/large)
            if large_domain == 'AVO':
                total_loss = w_avo * loss_large + w_p3 * loss_small + lambda_mmd * loss_align
            else:
                total_loss = w_p3 * loss_large + w_avo * loss_small + lambda_mmd * loss_align
...
def run_nested_cv_ablation1(logger: logging.Logger, channels: List[str]) -> Dict[str, float]:
    ...
    for repeat in range(NESTED_CV_REPEATS):
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X_all, y_all)):
            ...
            # Train fold model with ablation 1
            model = ablation1_train_fold(
                logger,
    ...
    log_section_header(logger, "CROSS-VALIDATION RESULTS (ABLATION 1: EQUAL WEIGHTS)")
...
def main():
    ...
    try:
        ...
        # Logger setup
        logger = setup_logger('Ablation1_AVOsmall_EqualWeights', create_file=True)
        log_section_header(logger, "ABLATION 1: Equal Weights for All Trials")

        # Log configuration summary
        log_configuration(logger, {
            'ablation': 'Equal Weights (w_P3 = w_AVO = 1.0)',
        ...
        # Run Ablation 1 with proper cross-validation
        log_section_header(logger, "Running Nested Cross-Validation with Ablation 1")
        results = run_nested_cv_ablation1(logger, channels)
        ...
        if 'detailed_fold_results' in results:
            ...
            csv_filename = f'ablation_results_AVOsmall/ablation1_equal_weights_detailed_{timestamp}.csv'
            ...
            summary_filename = f'ablation_results_AVOsmall/ablation1_equal_weights_summary_{timestamp}.csv'
            summary_df.to_csv(summary_filename, index=False)
            logger.info(f"Summary statistics saved to: {summary_filename}")
            print(f"Summary statistics saved to: {summary_filename}")

        print("\\n--- Ablation 1 Complete (Equal Weights) ---")
        ...
        print(f"\\n--- Ablation 1 Experiment Failed: {e} ---")
        if logger:
            cleanup_failed_log(logger)
        raise
    except KeyboardInterrupt:
        print("\\n--- Ablation 1 Experiment Interrupted by User ---")
        if logger:
            cleanup_failed_log(logger)
        raise
