...
def get_symmetric_adjustments(n_train_a: int, n_train_b: int) -> Tuple[float, float, int]:
    ...
    lambda_mmd = 0.400 if overall_ratio < 2.0 else (0.800 if overall_ratio < 4.0 else 1.200)
    warmup = max(2, min(10, int(0.193 * MAX_EPOCHS)))
...
def tfdwt_train_fold(
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        w_small_target = min(w_small_target, 5.6)
        w_large_target = 1.0
        ...
        while True:
            ...
            total_loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=3.6)
            optimizer.step()
